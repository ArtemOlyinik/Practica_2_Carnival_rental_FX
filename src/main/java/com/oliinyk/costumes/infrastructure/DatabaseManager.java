...
public class DatabaseManager {
    ...
    public static void initialize(String url, String user, String password) {
        ...
        // Flyway не може сканувати classpath у named JPMS-модулі через обмеження ClassLoader.getResources().
        ...
        java.net.URL migrationDirUrl =
                DatabaseManager.class.getResource("/db/migration/");
        String migrationLocation;
        if (migrationDirUrl != null && "file".equals(migrationDirUrl.getProtocol())) {
            try {
                migrationLocation =
                        "filesystem:"
                                + java.nio.file.Paths.get(migrationDirUrl.toURI())
                                        .toAbsolutePath();
